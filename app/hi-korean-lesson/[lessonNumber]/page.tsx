...
export default function HIKoreanLessonWordsPage({ params }: { params: { lessonNumber: string } }) {
  const [searchTerm, setSearchTerm] = useState("")
  const [words, setWords] = useState<Word[]>([])

  useEffect(() => {
    const lessonKey = `HI Korean 1A - Unit ${params.lessonNumber}`
  ...
  const unitTitle = unitTitles[params.lessonNumber] || `Unit ${params.lessonNumber}`
    ...
    <div className="min-h-screen bg-[#121212] text-[#EAEAEA] py-20">
      <div className="container mx-auto px-4">
        ...
        <h1 className="text-4xl font-bold mb-8">
          HI Korean 1A - Unit {params.lessonNumber}: {unitTitle}
        </h1>
        ...
        {filteredWords.length === 0 && (
          <div className="text-center py-8">
            <p className="text-[#EAEAEA]/60">No words found matching your search.</p>
          </div>
        )}
      </div>
    </div>
